...
const requestViewUtility = {
    ...
    openRespondRequestDisplay: function (tableRow) {
        if (respondRequestCheck) {
            ...
            pageUtility.attachTitleElement('h4', "Responding to request, close to choose another", respondFormContainer);
            pageUtility.generateNewLine(1, respondFormContainer);
            pageUtility.attachSelectOptions('status', "", ['Approve', 'Deny'], respondFormContainer);
    ...
    displayNewPendingRequests: function () {
        ...
        pageUtility.generateSingleTableRowCleanPender(newPendingRequestData, table, [0, 1, 6, 7]);
    },
...
function getUserLogin() {
    ...
    if(error) userForm.removeChild(error);
...
function failedLogin() {
    document.getElementById('formContainer').className = 'failedLogin';
    let error = document.createElement('h2');
    error.innerText = 'Incorrect';
    error.id = 'errorText';
    document.getElementById('formContainer').appendChild(error);
}
...
const userRequestUtility = {
    getCreateRequest: function () {
        ...
        let error = document.getElementById('errorText');
        if(error) requestForm.removeChild(error);
        let message = requestForm.elements[1].value;
        let type = requestForm.elements[0].value;
        ...
        if (message && type && amount && file) {
            ...
            requestForm.className = 'invalidInputForm';
    ...
    getIndividualEmployeeRequest: function () {
        individualEmployeeFormContainer.className = "";
        let error = document.getElementById('errorTexts');
        if(error) individualEmployeeFormContainer.removeChild(error);
        let id = individualEmployeeFormContainer.elements[0].value;

        if (id) {
            getIndividualEmployeeRequest(id);
        } else {
            individualEmployeeFormContainer.className = 'invalidInputForm';
            let error = document.createElement('h3');
            error.id = 'errorTexts';
            error.innerText = 'Invalid Input';
            individualEmployeeFormContainer.appendChild(error);
        }
...
async function getIndividualEmployeeRequest(id) {
    ...
    try {
        ...
        let error = document.createElement('h3');
        error.id = 'errorTexts';
        error.innerText = 'Employee Not Found';
        individualEmployeeFormContainer.appendChild(error);
    }
}
